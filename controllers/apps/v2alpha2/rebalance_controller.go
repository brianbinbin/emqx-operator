...
func rebalanceStatusHandler(emqx client.Object, rebalance *appsv2alpha2.Rebalance, requester innerReq.RequesterInterface,
	...
	case "":
		...
		_ = rebalance.Status.SetProcessing(appsv2alpha2.RebalanceCondition{
			Type:   appsv2alpha2.RebalanceConditionProcessing,
			Status: corev1.ConditionTrue,
		})
	case appsv2alpha2.RebalancePhaseProcessing:
		...
		if len(rebalanceStates) == 0 {
			_ = rebalance.Status.SetCompleted(appsv2alpha2.RebalanceCondition{
				Type:   appsv2alpha2.RebalanceConditionCompleted,
				Status: corev1.ConditionTrue,
			})
			rebalance.Status.RebalanceStates = nil
		}
	...
	case appsv2alpha2.RebalancePhaseFailed, appsv2alpha2.RebalancePhaseCompleted:
		rebalance.Status.RebalanceStates = nil
	default:
		panic("unknown rebalance phase")
	}
...
func getRequestBytes(rebalance *appsv2alpha2.Rebalance, nodes []string) []byte {
...
func getEmqxNodes(emqx client.Object) ([]string, error) {
	...
	} else if e, ok := emqx.(*appsv2alpha2.EMQX); ok {
